...
def createDepth(numberOfPointsInLine):
    ...
    for i in range(0,100):
        row=np.linspace(0,1,numberOfPointsInLine)
...
def flatCoords(x,y,z):
    fullArr=[]
    colors=[]
    red=np.linspace(0,1,np.shape(x)[0])
    green=np.linspace(0,1,np.shape(x)[1])

    for (rowx, rowy, rowz,color_red) in zip(x, y, z,red):
        for (it_x, it_y, it_z,color_green) in zip(rowx, rowy, rowz,green):  
            fullArr.append([it_x, it_y, it_z] )
            colors.append([color_red,color_green,0])
