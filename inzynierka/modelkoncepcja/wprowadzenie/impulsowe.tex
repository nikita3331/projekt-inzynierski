...
\subsection{Phase-based LIDAR}
Mierzący przesunięcie fazowe wiązki odbitej od obiektu (ang. \textit{Phase-based LIDAR}). Główną zasadą wykorzystywaną w tym skanerze jest fakt, że przy odbiciu od powierzchni faza światła zostaje przesunięta \cite{wehr1999airborne}. Mając również na uwadze, iż różnica przesunięcia fazowego pomiędzy wiązką odbitą, a wiązką wysłaną jest proporcjonalna do odległości przebytej przez światło można otrzymać następujące wzory \cite{articleLidar}.
...
d jest odległością zmierzonego punktu od odbiornika. c to prędkość światła. $f$ jest częstotliwością sygnału referencyjnego. $\phi$ to przesunięcie fazowe pomiędzy sygnałem referencyjnym, a odebranym przez odbiornik.
...
Skaner Benewake TF02 jest urządzeniem wykorzystującym pomiar przesunięcia fazowego do obliczania odległości (ang. \textit{indirect time-of-flight}). Jego charakterystyki oraz wygląd zostały przedstawione w tabeli ~\ref{tab:benewakeTab2}.
...
\begin{tabular}{ |c|c| c| }
 ...
 {\small Zasięg} & {\small 0.4 m - 22 m} & {\small 0.1 m-180 m}\\ 
  ...
  \hline
     {\small  Kąt wykrycia wiązki światła  } & {\small 3 \degree} & {\small 0.5 \degree}   \\  
  \hline
    
   {\small Błąd pomiaru } & {\small $\pm$5 cm przy odległości do 5 m, 2\% powyżej 5 m} & {\small $\pm$10 cm przy ogległości do 10 m, 1\% powyżej 10 m}  \\  
  \hline
